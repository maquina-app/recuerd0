...
class Workspaces::ArchivesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "create archives a workspace" do
    ...
    assert_equal I18n.t("workspaces/archives.create.created", name: workspace.name, raise: true), flash[:notice]
  end

  test "destroy unarchives a workspace" do
    workspace = workspaces(:archived)
    delete archive_workspace_url(workspace)
    # 303, or Turbo replays the DELETE against /workspaces, which is unrouted.
    assert_response :see_other
    assert_not workspace.reload.archived?
    assert_equal I18n.t("workspaces/archives.destroy.destroyed", name: workspace.name, raise: true), flash[:notice]
  end
end
